...
export async function loadRules(): Promise<Rule[]> {
	...
	try {
		...
		rules.push(
			...fallbackRules
				.filter((result): result is PromiseFulfilledResult<Rule> => result.status === "fulfilled")
...
export async function getCategoriesWithRulesNoStats(): Promise<
	(Category & {
		rules: (Rule & { upvotes: number; downvotes: number; viewCount: number; copyCount: number })[];
	})[]
> {
	const allCategories = await getCategories();
	const allRules = await loadRules();
...
export async function getCollectionBySlug(
	slug: string,
): Promise<(Collection & { ruleDetails: Rule[] }) | null> {
...
export async function getCollectionsWithRules(): Promise<
	(Collection & { ruleDetails: Rule[]; ruleCount: number })[]
> {
	const allRules = await loadRules();
